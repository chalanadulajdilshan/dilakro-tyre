...
if (isset($_POST['create'])) {
    ...
    foreach ($items as $item) {
        ...
        if (substr($item['code'], 0, 2) !== 'SI' && substr($item['code'], 0, 2) !== 'SV') {
            ...
            // Services (SV) - no stock management, just use the service price as cost
            $SERVICE = new Service($item['item_id']);
            $final_cost_item = 0; // Services typically have no cost, or you can set a default
            $final_cost += $final_cost_item;
        }
    ...
    if ($invoiceResult) {
        ...
        foreach ($items as $item) {
            ...
            if (substr($item['code'], 0, 2) !== 'SI' && substr($item['code'], 0, 2) !== 'SV') {
                ...
                // Service (SV) - no physical stock, just record the service
                $SALES_ITEM->service_item_code = $item['item_id'];
                $SALES_ITEM->quantity = $item['qty'];
                $qty_for_total = $item['qty'];
            ...
            // Store item name with code prefix, ARN ID and department for cancellation tracking
            ...
            $SALES_ITEM->serial_number = isset($item['serial_number']) ? $item['serial_number'] : '';
            $SALES_ITEM->created_at = date("Y-m-d H:i:s");
            $SALES_ITEM->create();

            // Only process stock management for regular items (not SI or SV)
            if (substr($item['code'], 0, 2) !== 'SI' && substr($item['code'], 0, 2) !== 'SV') {
    ...
    if (is_array($result) && $result['success']) {
        ...
        foreach ($items as $item) {
            ...
            if ($item['item_code'] != 0) {
                ...
                // Check if it's a Service Item (SI) or Service (SV) by checking the item_name prefix
                $isServiceItem = (strpos($item['item_name'], 'SI/') === 0);
                $isService = (strpos($item['item_name'], 'SV/') === 0);
                
                if ($isServiceItem && $item['service_item_code'] != 0) {
                    // Service Item (SI) - restore quantity
                ...
                // Services (SV) don't have stock to restore - no action needed
            }

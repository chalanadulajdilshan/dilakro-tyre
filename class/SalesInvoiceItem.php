...
class SalesInvoiceItem
{
    ...
    public function getItemsByInvoiceId($invoice_id)
    {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            // Extract code from item_name if it's in new format (CODE|NAME|ARN:xxx|DEPT:xxx)
            $extractedCode = $this->extractItemCode($row['item_name']);
            ...
            if ($row['item_code'] != 0) {
                ...
                // Use the code extracted from item_name (SV/xxxx or SI/xxxx)
                $row['item_code_name'] = $extractedCode;
            } else {
                // Fallback: try to get from ServiceItem
                $service_item_master = new ServiceItem($row['service_item_code']);
                $row['item_code_name'] = $service_item_master->item_code ?? '';
            }
            
            // Extract clean item name for display (remove code prefix and ARN metadata)
    ...
    // Helper method to extract item code from item_name (new format: CODE|NAME|ARN:xxx|DEPT:xxx)
    private function extractItemCode($itemName)
    {
        // Check if item_name starts with SV/ or SI/ (service or service item code)
        if (preg_match('/^(SV\/\d+|SI\/\d+)/', $itemName, $matches)) {
            return $matches[1];
        }
        return null;
    }
    
    // Helper method to extract clean item name without code prefix and ARN metadata
    private function extractCleanItemName($itemName)
    {
        ...
        // Check if there's a code prefix (SI/ or SV/ or regular item code)
        // Format: CODE|NAME - extract just the NAME part
